...
echo "$($PSQL "SELECT DISTINCT name FROM teams LEFT JOIN games ON (winner_id=team_id OR opponent_id=team_id) WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM games LEFT JOIN teams ON winner_id=team_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM games LEFT JOIN teams ON team_id=winner_id WHERE round='Final' ORDER BY year")"
